...
def main():
    ...
    learning_rate = (
        ...
        .get("lr_scheduler", {})
        .get("train_epochs", "Unknown")
    )
    optimizer_name = (
        config.get(model_name, {}).get("optimizer", {}).get("name", "Unknown")
    )
    scheduler_name = (
        config.get(model_name, {})
        .get("lr_scheduler", {})
...
def train(
    ...
    # Training parameters
    # num_epochs = (
    #     config.get(config["model"], {})
    #     .get("lr_scheduler", {})
    #     .get("train_epochs", 50)
    # )
    num_epochs = config["model"]["lr_scheduler"]["train_epochs"]
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch_data in enumerate(train_loader):
            ...
            # Power = real^2 + imag^2
            audio_power = torch.sqrt(
                audio[0, 0, 1, :] ** 2 + audio[0, 1, 1, :] ** 2
            )  # [F]
            ...
            gs_main = GridSpec(
                1, 2, figure=fig, width_ratios=[1, 1], wspace=0.43
            )
