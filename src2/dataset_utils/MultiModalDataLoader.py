...
def create_dataloaders(config):
    ...
    task_name = (
        ...
        or config.get("task")
        or config.get("classification_task")
    )
    if task_name:
        ...
        available_tasks = sorted(
            [k for k in config.keys() if k.endswith("_classification")]
        )
        if available_tasks:
            ...
            raise ValueError(
                "No classification task found in config (expected *_classification section)."
            )
...
def create_single_dataloader(
    ...
    # Create dataset
    dataset = MultiModalDataset(index_file=index_file, num_classes=num_classes)
    ...
    logging.info(
        f"{split.capitalize()} samples: {len(dataset)}, batches: {len(dataloader)}"
    )
...
def create_pretrain_dataloader(config):
    ...
    subset_ratio = training_configs[training_config_name]["pretrain_subset_ratio"]
    ...
    logging.info(f"Pretrain dataset: {len(dataset)} samples, {len(loader)} batches (batch_size={batch_size}, drop_last=True)")
    return loader
