...
/* ============================================================================
 * Weight-only int8 per-channel kernels (float activations + float MAC).
 *
 * Inputs and outputs are float32. Weights are stored int8 with a per-output-
 * channel symmetric scale (zero-point = 0). Each weight is dequantized on-the-
 * fly as scale[oc] * (float)int8_weight inside the MAC. No activation
 * quantization anywhere; arithmetic is float throughout.
 *
 * Matches the existing conv2d_nhwc_float_input_int8_weight_per_channel kernel
 * above; adds a depthwise variant and a dense variant.
 * ============================================================================ */

/**
 * Depthwise Conv2D NHWC: float activations, int8 depthwise weights, per-channel
 * symmetric scales -> float output. Used for both Conv2d depthwise and Conv1d
 * depthwise (H = 1 wrap).
 *
 * in:    [in_h, in_w, channels]  float32
 * filt:  [k_h, k_w, channels]    int8 (NHWC depthwise, one filter per channel)
 * bias:  [channels]              float32 or NULL
 * weight_scales: [channels]      float32 symmetric scale per channel
 * out:   [out_h, out_w, channels] float32
 */
static inline void depthwise_conv2d_nhwc_float_input_int8_weight_per_channel(
            ...
            for (int c = 0; c < channels; ++c) {
                ...
                for (int kh = 0; kh < k_h; ++kh) {
                    ...
                    for (int kw = 0; kw < k_w; ++kw) {
                        int iw = ow * stride_w + kw - pad_w;
                        if (iw < 0 || iw >= in_w) continue;
                        float x = in[((ih * in_w + iw) * channels) + c];
                        float w = (float)filt[((kh * k_w + kw) * channels) + c] * scale_c;
                        acc += x * w;
                    }
                }
                if (bias != NULL) acc += bias[c];
 ...
 * W:  [in_features, out_features]      int8 row-major (same layout as float
 *                                      `dense` and int8 `dense_int8_per_channel`)
 * b:  [out_features]                   float32 or NULL
 * weight_scales: [out_features]        float32 symmetric scale per output
 * y:  [out_features]                   float32
 */
    ...
    float* y)
{
    for (int o = 0; o < out_features; ++o) {
        ...
        const int8_t* w_col = W + o; /* W[i * out_features + o] */
        for (int i = 0; i < in_features; ++i) {
